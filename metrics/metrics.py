...
def compute_au_prc(y_true,df_score):
    ...
    ### RPCA OCSVM
    y_scores_pos = df_score["rpca_ocsvm-Train"]
    y_scores_neg = df_score["rpca_ocsvm-Test"]
    y_score = np.concatenate((y_scores_pos,y_scores_neg))
    rpca_ocsvm_prc = au_prc(y_true,y_score)
    ...
    y_scores_pos = df_score["cae_ocsvm-linear-Train"]
    y_scores_neg = df_score["cae_ocsvm-linear-Test"]
    y_score = np.concatenate((y_scores_pos,y_scores_neg))
    cae_ocsvm_lin_prc = au_prc(y_true,y_score)

    ### OCSVM- RBF
    y_scores_pos = df_score["cae_ocsvm-rbf-Train"]
    y_scores_neg = df_score["cae_ocsvm-rbf-Test"]
    y_score = np.concatenate((y_scores_pos,y_scores_neg))
    cae_ocsvm_rbf_prc = au_prc(y_true,y_score)
    ...
    return [ocnn_lin_prc,ocnn_sig_prc,ocnn_relu_prc,cae_ocsvm_lin_prc,cae_ocsvm_rbf_prc,ae_svdd_lin_prc,ae_svdd_rbf_prc,ocsvm_lin_prc,ocsvm_rbf_prc,rpca_ocsvm_prc,isolation_forest_prc]
def compute_au_roc(y_true, df_score):
    ...
    y_scores_neg = df_score["rpca_ocsvm-Test"]
    y_score = np.concatenate((y_scores_pos, y_scores_neg))
    rpca_ocsvm_roc = au_roc(y_true, y_score)
    ...
    y_scores_neg = df_score["cae_ocsvm-linear-Test"]
    y_score = np.concatenate((y_scores_pos, y_scores_neg))
    cae_ocsvm_lin_roc = au_roc(y_true, y_score)

    ### OCSVM- RBF
    y_scores_pos = df_score["cae_ocsvm-rbf-Train"]
    y_scores_neg = df_score["cae_ocsvm-rbf-Test"]
    y_score = np.concatenate((y_scores_pos, y_scores_neg))
    cae_ocsvm_rbf_roc = au_roc(y_true, y_score)
    ...
    return [ocnn_lin_roc, ocnn_sig_roc,ocnn_relu_roc,ocsvm_lin_roc, cae_ocsvm_lin_roc,cae_ocsvm_rbf_roc,ae_svdd_lin_roc,ae_svdd_rbf_roc,ocsvm_rbf_roc, rpca_ocsvm_roc,isolation_forest_roc]
def compute_prec_at_10(y_true, df_score):
    ...
    y_scores_neg = df_score["rpca_ocsvm-Test"]
    y_score = np.concatenate((y_scores_pos, y_scores_neg))
    rpca_ocsvm_prec = compute_precAtK(y_true, y_score)
    ...
    y_scores_neg = df_score["cae_ocsvm-linear-Test"]
    y_score = np.concatenate((y_scores_pos, y_scores_neg))
    cae_ocsvm_linear_prec = compute_precAtK(y_true, y_score)

    y_scores_pos = df_score["cae_ocsvm-rbf-Train"]
    y_scores_neg = df_score["cae_ocsvm-rbf-Test"]
    y_score = np.concatenate((y_scores_pos, y_scores_neg))
    cae_ocsvm_rbf_prec = compute_precAtK(y_true, y_score)
    ...
    return [ocnn_lin_prec, ocnn_sig_prec,ocnn_relu_prec,cae_ocsvm_linear_prec,cae_ocsvm_rbf_prec,ae_svdd_lin_prec,ae_svdd_rbf_prec,ocsvm_lin_prec, ocsvm_rbf_prec,rpca_ocsvm_prec,isolation_forest_prec]
