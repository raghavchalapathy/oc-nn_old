...
def oneClassNN(incoming,v,rho,hidden_layer,output_layer, placeholder='default', optimizer='adam',
               loss='OneClassNN_Loss', metric='default',
    ...
    if isinstance(loss, str):
        loss = objectives.get(loss)(incoming,placeholder,v,rho,hidden_layer,output_layer)
    # Check if function
...
def regression_RobustAutoencoder(incoming,mue, hidden_layer,decode_layer,placeholder='default',optimizer='adam',
               loss='categorical_crossentropy', metric='default',
               learning_rate=0.001, dtype=tf.float32, batch_size=64,
               shuffle_batches=True, to_one_hot=False, n_classes=None,
               trainable_vars=None, restore=True, op_name=None,
    ...
    # Building other ops (loss, training ops...)
    if isinstance(loss, str):
        loss = objectives.get(loss)(incoming, placeholder, mue,hidden_layer,decode_layer)
    # Check if function
    elif hasattr(loss, '__call__'):
        try:
            loss = loss(incoming, placeholder)
